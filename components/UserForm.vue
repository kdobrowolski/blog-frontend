<template>
  <form class="User_form" @submit.prevent="submit">
    ...
    <p v-if="success" class="form_success">Dodano nowego użytkownika pomyślnie!</p>
    <p v-if="errors.existUser" class="form_error">Istnieje juz taki uzytkownik!</p>
    <Button element="submit" v-if="!userExist" value="Utwórz" />
  </form>
</template>

<script>
import { userValidation } from '../helpers/validationForms';
...
export default {
  ...
  methods: {
    ...
    async submit(e) {
      ...
      const validation = userValidation(user, this.repeatPassword);
